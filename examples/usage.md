...
```bash
python scripts/xqapi_image.py gen "一只戴墨镜的柴犬" -m <MODEL>
```
...
```bash
python scripts/xqapi_image.py gen "水墨风格的远山" -m <MODEL> -r 2k -o out/shan.png
```

## 3. 4k 长任务（走异步，别等 600s 超时）

```bash
python scripts/xqapi_image.py gen "超写实机械蜂鸟，微距" -m <MODEL> -r 4k --async --outdir out/bird/
```

## 4. 多张候选

```bash
python scripts/xqapi_image.py gen "四个不同的 App 图标，扁平风" -m <MODEL> -n 4 --outdir out/icons/
```

## 5. 图生图 —— 多参考图（multipart 路）

```bash
python scripts/xqapi_image.py edit "保留主体，背景换成雪山日落" -m <MODEL> \
...
python scripts/xqapi_image.py edit "把猫换成柯基" -m <MODEL> \
...
```bash
python scripts/xqapi_image.py edit "把背景换成雪山" -m <MODEL> \
...
```bash
python scripts/xqapi_image.py edit "统一改成水彩风格" -m <MODEL> \
...
```bash
python scripts/xqapi_image.py gen "极简线框猫头" -m <MODEL> --b64-json -o out/cat_line.png
```
...
```bash
python scripts/xqapi_image.py gen "海报" -m <MODEL> \
...
with open("cat.png", "rb") as f:
    r = client.images.edit(model="<MODEL>", image=f, prompt="把背景换成雪山")
```
...
foreach ($p in $prompts) {
    $i++
    python scripts/xqapi_image.py gen $p -m <MODEL> -r 2k --async -o "out/batch-$i.png"
}
```
